        ...
        <div class="row justify-content-center">
          <div class="col-lg-6 text-center">
            <h1 class="mb-4 heading text-white" data-aos="fade-up" data-aos-delay="100"
              style="font-family: 'Almarai', serif;">تسجيل الدخول</h1>
          </div>
        </div>
      </div>
    </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="form-box" style="text-align: right">
          ...
          <div class="mb-3">
            ...
            <x-input id="email" class="form-control" type="email" name="email" :value="old('email')" required autofocus
              autocomplete="username" style="text-align: right" placeholder="البريد الإلكتروني" />
            @error('email')
      ...
      @enderror
          ...
          <div class="mb-3">
            ...
            <x-input id="password" class="form-control" type="password" name="password" required
              autocomplete="current-password" style="text-align: right" placeholder="كلمة المرور" />
            @error('password')
        <span class="text-danger">{{ $message }}</span>
      @enderror
          </div>
